export default class FormValidator { //класс валидатор формы
  ...
  //Включение и выключение кнопки на основе данных от hasInvalidInput
    ...
    enableValidation(toggle) {
        if(toggle) this._setEventListeners();       // если true, то включить валидацию (запускаем метод набора слушателей)
        else elementFormProfile.removeEventListener("input"); // иначе выключить (удаляем слушатель)
